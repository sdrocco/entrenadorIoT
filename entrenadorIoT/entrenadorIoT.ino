...
int activar(int maxT, int cRojo, int cVerde, int cAzul, int tono, int maxDistCM){
  ...
  pixels.show();
  Serial.println("Enciende Leds");

  //Activamos Buzzer
  tone(BUZZER,tono);
  Serial.println("Activa Buzzer");
  ...
  Serial.print("Tiempo Inicio Activacion: ");
  Serial.print(tiempo);
  Serial.print(" - Tiempo Max Act: ");
  Serial.print(maxT);
  ...
  while (tiempo+maxT > millis()){
    ...
    Serial.print("EchoTiempo: ");
    Serial.println(echoTiempo);
  ...
  if(activacion){ 
    Serial.print("Se activo luego de: ");
    Serial.println(tiempoActivacion);
    return tiempoActivacion;
  }else{
    Serial.println("No se logro activar el dispositivo a Tiempo");
    pixels.clear();
    pixels.show();
    noTone(BUZZER);
    return 0;
    ...
    if(procesarPaqueteUDP()>0){
      ...
      if(tactivacion>0){
        ...
        if(dataRespuesta>0){
          ...
          respuestaUDP= (String)DISPOSITIVO+"#"+(String)tactivacion+"##";
          UDP.beginPacket(UDP.remoteIP(),UDP.remotePort());
          UDP.print(respuestaUDP);
          UDP.endPacket(); 
          Serial.print("Trama Respuesta Enviada:");
          Serial.println(respuestaUDP);
        ...
        if(dataRespuesta>0){
          //Enviar respuesta al clliente
          UDP.beginPacket(UDP.remoteIP(),UDP.remotePort());
          respuestaUDP= (String)DISPOSITIVO+"#0##";
          UDP.beginPacket(UDP.remoteIP(),UDP.remotePort());
          UDP.print(respuestaUDP);
